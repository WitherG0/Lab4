        ...
        static void Main(string[] args)
        {
			...
			for (int i = 0; i < trackList.Length; i++)
			{
				...
				foreach (Match m in min)
				{
					...
					/*Console.WriteLine("Минуты: " + digitMin);*/
					sum += digitMin * 60;
				}
				foreach (Match s in sec)
				{
					digitSec = int.Parse(s.Value);
					/*Console.WriteLine("Секунды: " + digitSec);*/
					sum += digitSec;
				}
				Console.WriteLine();
			...
			Console.WriteLine();
			Console.WriteLine("Суммарное время " + sum + " с");
			Console.WriteLine("Самая короткая " + shortestSong);
			Console.WriteLine("Самая длинная " + longestSong);
			Console.WriteLine("Песни с минимальным разрывом по длине: {0} и {1}", trackList[firstTrackIndex], trackList[secondTrackIndex]);
			Console.ReadKey();
		}
    }
}
